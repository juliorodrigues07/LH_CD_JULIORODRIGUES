...
st.title('Static Graphs About American Properties')
...
graph1 = static_dataset.groupby(['Neighborhood'])[['Monthly Reviews']].mean().reset_index()
fig1 = px.bar(graph1, x='Neighborhood', y='Monthly Reviews', color='Neighborhood', title="Average montlhy reviews per district")
col1.plotly_chart(fig1, use_container_width=True)

graph2 = static_dataset.groupby(['Neighborhood', 'Room Type'])[['Minimum Nights']].mean().reset_index()
fig2 = px.bar(graph2, x='Neighborhood', y='Minimum Nights', color='Room Type', title="Average minimum nights required")
